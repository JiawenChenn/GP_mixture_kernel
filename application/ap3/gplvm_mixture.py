...
class bGPLVM(BayesianGPLVM):
    def __init__(self, n, data_dim, latent_dim, n_inducing, pca=False):
        ...
        #
        # Kernel (actding on latent dimensions)
        self.mean_module = ZeroMean(ard_num_dims=latent_dim)
        #########################################
        ##### Here using the mixture kernel #####
        #########################################
        self.covar_module = mixture_kernel.Mixed_kernel_GPLVM(data_covar_module_type="matern_different_smoothness",num_tasks=1,ard_num_dims=latent_dim)
        #
    def forward(self, X):
...
# set latent dimension using input
...
parameter=torch.cat((predict_weight,
                        1/model.covar_module.matern1.base_kernel.lengthscale.squeeze(0),1/model.covar_module.matern2.base_kernel.lengthscale.squeeze(0),1/model.covar_module.matern3.base_kernel.lengthscale.squeeze(0),
                        model.covar_module.matern1.outputscale.unsqueeze(0),model.covar_module.matern2.outputscale.unsqueeze(0),model.covar_module.matern3.outputscale.unsqueeze(0))).detach().cpu()

print(parameter)
parameter = np.array(parameter)
np.savetxt('./result/mixture/model_latent'+str(latent_dim)+'_para.txt',parameter)

# save the latent embedding
X = model.X.q_mu.detach().cpu().numpy()
np.savetxt('./result/mixture/X_latent'+str(latent_dim)+'.txt', X)

if latent_dim==2:
    plt.clf()
    plt.figure(figsize=(5,5))
    scatter_plot = sns.scatterplot(x=X[:,0], y=X[:,1], hue=labels, palette="tab10")
    plt.savefig("./gplvm_mixture.png",dpi=300)